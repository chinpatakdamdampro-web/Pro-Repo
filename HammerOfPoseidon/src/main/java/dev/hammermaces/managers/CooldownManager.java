...
public class CooldownManager {
    ...
    /** Reduces remaining cooldown by refundSeconds — used for Contingency partial refund. */
    public void reduceAbilityCooldown(UUID uuid, String maceId, String ability, int refundSeconds) {
        String key = key(uuid, maceId, ability);
        if (!cooldowns.containsKey(key)) return;
        cooldowns.put(key, cooldowns.get(key) - (refundSeconds * 1000L));
    }
